...
const SearchInput = ({
  ...
  const handleSelectChange = (value) => {
    setSelectedRamo(value); // Atualiza o ramo selecionado
    setSearchTerm(value);   // Atualiza o input de busca com o ramo selecionado
            ...
            <ul className="custom-select-dropdown">
              {filteredOptions.length > 0 ? (
                filteredOptions.map((ramo, index) => (
                  <li
                    key={index}
                    onClick={() => handleSelectChange(ramo)} // Atualiza o ramo selecionado
