...
function ListaGestao(props) {
      ...
      if (result.isConfirmed) {
        ...
        if (senhaDigitada === senhaCorreta) {
          ...
          localStorage.setItem(
            "additionalInfo",
            JSON.stringify({ ...additionalInfo, [clienteId]: null })
          );
          Swal.fire("Informações excluídas!", "", "success");
        } else {
          Swal.fire(
            "Senha incorreta!",
            "Você não tem permissão para excluir informações.",
            "error"
          );
        }
      }
  ...
  const addInfoManually = async (clienteId) => {
    ...
    if (result.isConfirmed) {
      ...
      localStorage.setItem(
        "additionalInfo",
        JSON.stringify({ ...additionalInfo, [clienteId]: { info, name } })
      );
    }
  };
  ...
  function formatarData(dataEncaminhamento) {
    if (
      typeof dataEncaminhamento === "string" &&
      dataEncaminhamento.includes("-")
    ) {
  ...
  useEffect(() => {
    ...
    const filterClientes = async () => {
      try {
        ...
        const filtered = await Promise.all(
          props.arrayClientes.map(async (cliente) => {
            try {
                ...
                const allNames =
                  cliente.cobrador === "bruno" ||
                  cliente.cobrador === "allan" ||
                  cliente.cobrador === "jhow" ||
                  cliente.cobrador === "ana" ||
                  cliente.cobrador === "talita" ||
                  cliente.cobrador === "karol" ||
                  cliente.cobrador === "ana.caroline" ||
                  cliente.cobrador === "alef";
                if (
                  (allNames && allVisu) ||
                  (cliente.cobrador === "bruno" &&
                    user &&
                    user.uid === "xwRvGUiEJWbGuMvCNHiHYzmw0Cu1") || // bruno
                  (cliente.cobrador === "allan" &&
                    user &&
                    user.uid === "BhHsQBvMqoSTKsohAy4Bg380v8a2") || // allan
                  (cliente.cobrador === "ana" &&
                    user &&
                    user.uid === "oadTRyuG9bSHfPArrfPEZ8VN5iQ2") || // ana
                    (cliente.cobrador === "jonas" &&
                      user &&
                      user.uid === "Hc2jp2W3zhX2vkEh9BV6pEXlGh33") ||
                  (cliente.cobrador === "talita" &&
                    user &&
              ...
              console.error(
                `Erro ao verificar arquivos do cliente ${cliente}:`,
                error
              );
              return null;
            }
        ...
        console.log("Clientes filtrados da gestao:", filtered);
            ...
            .map((cliente) => {
              ...
              return (
                <tr key={cliente.id} className="table-light">
                  ...
                  <td className="align-middle text-center">
                    <Link to={`/app/fichafinanceiro/${cliente.id}`}>
